...
exports.addTeacher = async (req, res) => {
  try {
    ...
    if (teacher) {
      return res.status(400).send('มีรหัสผู้สอนแล้ว');
    }
...
exports.UpdateTeacher = async (req, res) => {
  try {
    ...
    // Check if the provided username already exists for another teacher
    const existingTeacher = await User.findOne({ username, _id: { $ne: _id } }).exec();
    if (existingTeacher) {
      return res.status(400).send('มีรหัสผู้สอนแล้ว');
    }
